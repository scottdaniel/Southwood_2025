...
s2 %>%
  ...
  select(
    ...
    `Percent host reads`) %>%
  write_csv(here("Figures", "Supplemental Table 3.csv"))
```
...
s_Admit <- s_toTest %>%
  filter(timepoint %in% "Admit")

bc_sub <- dist_subset(bc, s_Admit$SampleID)
...
data.scores$SampleID <- rownames(data.scores)  # create a column of site names, from the rownames of data.scores
data.scores %<>%
  left_join(s_Admit %>% select(SampleID, study_group), join_by(SampleID))

grp.a <- data.scores[data.scores$study_group == "24h", ][chull(data.scores[data.scores$study_group == 
    "24h", c("NMDS1", "NMDS2")]), ]  # hull values for grp A
grp.b <- data.scores[data.scores$study_group == "72h", ][chull(data.scores[data.scores$study_group == 
    "72h", c("NMDS1", "NMDS2")]), ]  # hull values for grp B
grp.c <- data.scores[data.scores$study_group == "Med", ][chull(data.scores[data.scores$study_group == 
    "Med", c("NMDS1", "NMDS2")]), ]  # hull values for grp C

hull.data <- rbind(grp.a, grp.b, grp.c)  #combine grp.a and grp.b and grp.c
# head(hull.data)

ggplot() + 
  geom_polygon(data=hull.data,aes(x=NMDS1,y=NMDS2,fill=study_group,group=study_group),alpha=0.30) + # add the convex hulls
  geom_point(data=data.scores,aes(x=NMDS1,y=NMDS2,shape=NULL,colour=study_group),size=3) + # add the point markers
  scale_colour_manual(values=ann_colors$study_group) +
  scale_fill_manual(values = ann_colors$study_group) +
  theme_bw() +
  labs(colour = "Study group", fill = "Study group") + 
  theme(axis.text.x = element_blank(),  # remove x-axis text
        axis.text.y = element_blank(), # remove y-axis text
        axis.ticks = element_blank(),  # remove axis ticks
        axis.title.x = element_text(size=18), # remove x-axis labels
...
ggsave(here("Figures", "NMDS_taxa_admit_only_hull_plot.pdf"), useDingbats = F, height = 4, width = 5)
...
bc_sub <- dist_subset(bc, s_Discharge$SampleID)
...
data.scores$SampleID <- rownames(data.scores)  # create a column of site names, from the rownames of data.scores
data.scores %<>%
  left_join(s_Discharge %>% select(SampleID, study_group), join_by(SampleID))
...
ggsave(here("Figures", "NMDS_taxa_discharge_only_hull_plot.pdf"), useDingbats = F, height = 4, width = 5)
...
summaries_df <- rbind(
  s_toTest %>%
    do(adonisplus(
      ., distmat = bc, formula = distmat ~ Age + Sex + study_group * timepoint,
  ...
  s_toTest %>%
    do(adonisplus(
      ., distmat = jd, formula = distmat ~ Age + Sex + study_group * timepoint,
      sample_id_var = SampleID, rep_meas_var = subject_id,
      shuffle = c(study_group = "between", timepoint = "within"), permutations=perm)) %>%
...
We can compare group to group with Permanova as well.
...
summaries_df <- rbind(
  s_toTest %>%
    do(adonispost(
      ...
      sample_id_var = SampleID, rep_meas_var = subject_id, which = time_study,
      permutations=perm)) %>%
    mutate(metric = "Bray-Curtis"),
...
summaries_df <- summaries_df %>%
  ...
  filter(term %in% "time_study") %>%
  select(-term, -one_of(c("sumsq"))) %>%
  mutate(comparison = str_remove(comparison, " time_study")) %>%
  select(Distance, everything())

summaries_df %>%
  pander(split.cells = 20, digits = 2, caption = glue("Post-hoc tests for permutational analysis of variance of Bray-Curtis and Jaccard distances of taxa abundances. Formula: distance_matrix ~ Age + Sex + time_study"), emphasize.strong.rows = which(.$p.value<=0.05))

table_export(x = summaries_df, title = "Taxonomic_table_export")
