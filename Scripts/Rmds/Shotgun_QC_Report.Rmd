...
s_toPlot <- s %>%
  ...
  select(SampleID, SampleType, !!enquo(study_group_var), host_species) %>%
  arrange(host_species, get(study_group_var)) %>%
  arrange(SampleType) %>%
  droplevels()
...
props_toPlot %>%
  ...
  pheat_cluster_rows() %>%
  pheat_annotate_cols(s_toPlot) %>%
  pheat_save(filename = here("Figures", "Supplemental_Figure_S1.pdf"))
