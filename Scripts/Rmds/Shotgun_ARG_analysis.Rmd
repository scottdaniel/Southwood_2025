...
ARO counts from the CARD database were transformed in the following way: counts were divided by the million nonhost reads and the ARO length in kilobases, thus RPKM (Reads per Kilobase per Million reads). Zeros were then replaced by 1/2 the minimum value for any given ARO. Finally, a logarithm of base 2 was taken for the relative abundance to account for skewed distributions. We will restrict our initial analysis to the top 100 genes that are present (i.e. nonzero before transformation) in at least `r min_percent` of samples.
...
raremedian <- median(rowSums(ARO_matrix))
rarefull <- max(rowSums(ARO_matrix))
...
ggplot(rare_df, aes(x = Sample, y = Species, color = Site)) +
  ...
  geom_vline(xintercept = raremedian, linetype = "dashed") +
  geom_vline(xintercept = rarefull, linetype = "dashed") +
  theme_bw() +
...
## Testing richness on three differenct sampling levels

Note: These sampling levels are the counts of the ARO's not the read depth.

```{r}

richness_min <- tibble(Richness_ARO = rarefy(ARO_matrix, sample = raremax), SampleID = rownames(ARO_matrix), Sampling_depth = "Minimum = 2218")
richness_median <- tibble(Richness_ARO = rarefy(ARO_matrix, sample = raremedian), SampleID = rownames(ARO_matrix), Sampling_depth = "Median = 37045")
richness_full <- tibble(Richness_ARO = rarefy(ARO_matrix, sample = rarefull), SampleID = rownames(ARO_matrix), Sampling_depth = "Maximum = 85954")
my_df <- bind_rows(richness_min, richness_median, richness_full)

my_plot <- my_df %>%
  inner_join(s_toTest, by = "SampleID") %>%
  ggplot(aes(x = time_study, y = Richness_ARO)) + 
  geom_boxplot(outlier.alpha = 0) +
  facet_wrap(~Sampling_depth, nrow = 1, scales = "free") +
  geom_quasirandom() +
  theme_bw() +
  theme(axis.text.x = element_text(angle=45, hjust = 1)) +
  xlab(label = "TimeXStudy") + ylab("Average Richness (number of unique AROs)") +
  ggtitle(label = "Richness at various sampling depths")

my_plot

ggsave(here("Output", "richness of AROs horses diff sample depths.pdf"), height = 4, width = 7)

```

### Linear models

Minimum sampling depth = 2218.
...
Median sampling depth = 37,045.

```{r}
my_lm <- richness_median %>%
  inner_join(s_toTest, by = "SampleID") %>%
  nlme::lme(Richness_ARO ~ Age + Sex + time_study, data = ., random = ~1|subject_id)
...
Maximum sampling depth = 85,954 (i.e. no sampling is done)
...
CARD_rpkm_with_zeros <- CARD %>%
  ...
  mutate(kbase = nt_length / 1E3) %>%
  mutate(RPKM = count / millr / kbase) %>%
  mutate(rel_RPKM = RPKM / 1E6) %>% # so they are all between 0 and 1
  select(-database)
...
## Subsetting to Discharge group

And saved as `ZIBR_model_results_AROs_Discharge.csv`

```{r eval=FALSE, include=FALSE}

s_Discharge <- s_toTest %>% 
  filter(timepoint %in% "Discharge")

my_covariates <- s_Discharge %>% 
  select(SampleID, Age, Sex, study_group) %>%
  column_to_rownames(var = "SampleID") %>%
  mutate(Sex = ifelse(Sex %in% "F", 1, 2)) %>%
  mutate(study_group = case_when(study_group %in% "24h" ~ 1,
                                study_group %in% "72h" ~ 2,
                                study_group %in% "Med" ~ 3))
...
for (my_ARO in unique(CARD$ARO)) {
  ...
  Y <- CARD_rpkm_with_zeros %>%
  filter(ARO %in% my_ARO, SampleID %in% s_Discharge$SampleID) %>%
  select(SampleID, rel_RPKM) %>%
  column_to_rownames(var = "SampleID")
  ...
  zibr_fit <- zibr(logistic_cov = my_covariates, beta_cov = my_covariates, subject_ind = s_Discharge$subject_id, Y = Y, time_ind = my_time$timepoint, component_wise_test = T, quad_n = 30, verbose = F)
...
my_results_df %>%
  write_csv(here("Figures", "ZIBR_model_results_AROs_Discharge.csv"))

```

## Part of Supplemental Figure 4

```{r}

TEM_changes <- read_csv(here("Figures", "ZIBR_model_results_AROs_Discharge.csv")) %>%
  filter(description %in% c("TEM-117", "TEM-126", "TEM-192")) %>%
  filter(term %in% "study_group")

plot_df <- TEM_changes %>%
  mutate(Significant = case_when(beta_pvalue < 0.05 ~ "p.adj < 0.05",
                                 beta_pvalue > 0.05 ~ "Not significant")) %>%
  mutate(description = fct_reorder(description, beta_est))

g <- ggplot(plot_df, aes(x = beta_est, y = description)) +
  theme_bw() +
  ...
  labs(title = "",
       subtitle = "",
       x = "<-- Decreased vs. 24h   Increased vs. 24h -->",
       y = "ARO in Penem class") +
  scale_shape_manual(values = c(1,19))
plot(g)

fig_name = "Supplemental_Figure_S4_part2.pdf"
ggsave(here("Figures",fig_name), width = 4, height = 3, useDingbats = F)
print(glue("Figure saved as {fig_name}"))
...
report_fp <- here::here(script_dir, "Shotgun_ARG_analysis.Rmd")
